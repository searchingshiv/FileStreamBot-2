...
async def render_page(id, secure_hash, src=None):
    file = await FileStream.get_messages(int(Telegram.BIN_CHANNEL), int(id))
    file_data = await get_file_ids(FileStream, int(Telegram.BIN_CHANNEL), int(id))
    ...
    if tag in ["video", "audio"]:
        template_file = "FileStream/template/req.html"
    else:
